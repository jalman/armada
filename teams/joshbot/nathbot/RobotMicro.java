...
public class RobotMicro {
	...
	public static boolean luge(RobotController rc) {
		...
		int nearbyTeamCount = 0;
		for (Robot r : nearbyTeam) {
			try {
				if(rc.senseRobotInfo(r).type == RobotType.SOLDIER) nearbyTeamCount++;
		...
		rc.setIndicatorString(0, "team " + nearbyTeamCount + " enemy " + nearbyEnemies.length + "," + Clock.getRoundNum());
		
		if (nearbyTeamCount+1 >= nearbyEnemies.length) {
			if ((nearbyEnemies.length == 0 || nearbyTeamCount-1 >= nearbyEnemies.length) && enemiesInRange.length == 0) {
